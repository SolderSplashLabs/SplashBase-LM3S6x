...
// 2013/01/06 Modified by Carl Matthews - SolderSplash Labs Ltd - Added DEL processing so a default putty session can delete characters
    ...
    if(ulInts & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(MAP_UARTCharsAvail(g_ulBase))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                //
            	// CM : added 0x7F, putty default backspace char
